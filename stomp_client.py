...
class StompClient:
    ...
    def subscribe(self, destination, callback):
        """
        Subscribe to a destination and supply a callback that should be executed when a message is received on that
        destination
        """
...
class Dispatcher:
    ...
    def _transmit(self, command, headers, msg=None):
        """
        Marshals and transmits the frame
        """
        # Construct the frame
    ...
    def _parse_message(self, frame):
        ...
        # set body to None if there isn't a body
    ...
    def connect(self):
        ...
        headers['accept-version'] = ','.join(VERSIONS)
